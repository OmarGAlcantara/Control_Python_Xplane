...
# git: @OmarGAlcantara
#
# Description: This script is a modified version of the XPlaneConnect repo: https://github.com/nasa/XPlaneConnect.
#              It manages the communication with XPlane Simulator by receiving, unpacking and packing the necessary UDP sent data.
...
def DecodeDataMessage(message):
    ...
    ### IMPORTANT TO NOTICE !!  ###
    # Below is the definition of the "keys" under which each simulation variable is going to be named inside the unpacked dictionary
    # In order to call for the current unpacked value we must use the exact key inside double commas ""
    # Hint: A clue on the information order can be to look under which order the data is being displayed in the specific line on the XPlane Cockpit
    ...
    if type == 0:
        ...
        values["times"] = float(dataFLOATS[0])
    # elif type == 14:
    ...
    #  values["Brakes"]=round(dataFLOATS[1],3)
        ...
        values["xx"] = dataFLOATS[0]
        values["yy"] = dataFLOATS[1]
        values["zz"] = dataFLOATS[2]
        values["vx"] = dataFLOATS[3]
        values["vy"] = dataFLOATS[4]
        values["vz"] = dataFLOATS[5]
    elif type == 25:
